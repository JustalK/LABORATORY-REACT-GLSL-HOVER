...
export default class CurlNoiseMaterial extends THREE.ShaderMaterial {
  constructor() {
    super({
      uniforms: {
      ...
      void main() {
        vec2 newUV = vUv;
    	  float f = sin(uVelo * 1.6) * 0.5 + 0.5 * uVelo;
        vec3 curl = curlNoise(vec3(newUV,1.) *5. + uVelo  * 1.6);

        vec4 t0 = texture(uTexture, vec2(newUV.x,newUV.y + f * (curl.x) ) );
        vec4 t1 = texture(uTexture2, vec2(newUV.x,newUV.y + (1.0-f) * (curl.x) ));
    	  newUV.x += curl.x;
